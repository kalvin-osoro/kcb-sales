...
public class VoomaChannelDashboardVC {
    @Autowired
    private IVoomaChannelService voomaService;


    @PostMapping("/vooma-summary")
    public ResponseEntity<?> getSummary(@RequestBody DSRSummaryRequest model) {
        ...
        ArrayNode dsrSummary= voomaService.getDSRSummary(model);
    ...
    @PostMapping("/vooma-get-nearby-customers")
...
//          node.put("id",0);
    ...
    @PostMapping("/vooma-search-customers")
