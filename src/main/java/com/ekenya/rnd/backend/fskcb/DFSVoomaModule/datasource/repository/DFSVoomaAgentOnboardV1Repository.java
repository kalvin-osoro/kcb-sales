...
public interface DFSVoomaAgentOnboardV1Repository extends JpaRepository<DFSVoomaAgentOnboardV1,Long> {
    ...
    @Query(value = "SELECT * FROM dfs_vooma_Agent_onboardingV1 WHERE organisationName LIKE '%?1%'; OR businessPhoneNumber LIKE '%?1%' OR  accountName LIKE '%?1%' ", nativeQuery = true)
